...
class Rental():
    ...
    def set_option(self, option_type):
        rental_days = self._get_rental_days()
        selector = {
            lambda op: option_type == 'gps':
            GPS(rental_days),
            lambda op: option_type == 'baby_seat':
            BabySeat(rental_days),
            lambda op: option_type == 'additional_insurance':
            AdditionalInsurance(rental_days),
        }

        selected = [selector[o] for o in selector if o(option_type)]
        self._options.append(selected[0])

    def get_options(self):
        option_types = []

        for option in self._options:
            option_types.append(option.get_type())
    ...
    def get_actions(self):
        ...
        options_money_owner, options_money_company = self._get_options_money()
        total_money_in_options = options_money_owner + options_money_company

        driver = Driver(rental_price, total_money_in_options)
        owner = Owner(rental_price, commission_fees, options_money_owner)
    ...
    def _get_options_money(self):
        options_money_owner = 0
        options_money_company = 0

        if self._options is not None:
            for option in self._options:
                options_money_owner += option.get_money_to_owner()
                options_money_company += option.get_money_to_company()

        return options_money_owner, options_money_company
